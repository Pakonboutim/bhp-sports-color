...
function getColorStudents(color,sportId){if(COLORS.indexOf(color)<0)throw new Error('สีไม่ถูกต้อง');const sports=getCompetitionData().sports,sport=sports.find(s=>s.id===sportId);if(!sport)throw new Error('ไม่พบกีฬา');const students=studentObjects_().filter(s=>s.color===color);const selectedIds=rows_(sheet_(ATHLETES_SHEET,HEADERS.Athletes)).filter(r=>String(r[0])===sportId&&String(r[1])===color).map(r=>String(r[2]));return{students:students,selectedIds:selectedIds,limit:sport.athleteLimit}}
function saveAthletes(sportId,color,studentIds,userType){if(COLORS.indexOf(color)<0)throw new Error('สีไม่ถูกต้อง');if(!Array.isArray(studentIds))throw new Error('ข้อมูลนักกีฬาไม่ถูกต้อง');const sport=getCompetitionData().sports.find(s=>s.id===sportId);if(!sport)throw new Error('ไม่พบกีฬา');if(studentIds.length>sport.athleteLimit)throw new Error('นักกีฬาเกินจำนวนที่กำหนด');const allowed=studentObjects_().filter(s=>s.color===color),map={};allowed.forEach(s=>map[s.id]=s);studentIds.forEach(id=>{if(!map[String(id)])throw new Error('พบนักเรียนที่ไม่ใช่สี '+color)});const lock=LockService.getScriptLock();lock.waitLock(10000);try{const sh=sheet_(ATHLETES_SHEET,HEADERS.Athletes);for(let i=sh.getLastRow();i>=2;i--){const r=sh.getRange(i,1,1,2).getValues()[0];if(String(r[0])===sportId&&String(r[1])===color)sh.deleteRow(i)}studentIds.forEach(id=>{const s=map[String(id)];sh.appendRow([sportId,color,s.id,(s.prefix||'')+s.firstName+' '+s.lastName,s.level+'/'+s.room,now_()])});audit_('SAVE_ATHLETES',userType||'Color Manager',{sportId:sportId,color:color,count:studentIds.length});return{success:true,saved:studentIds.length}}finally{lock.releaseLock()}}
