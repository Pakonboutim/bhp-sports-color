...
const STUDENT_PHOTOS_SHEET='StudentPhotos';
...
function photoMap_(){const map={};rows_(sheet_(STUDENT_PHOTOS_SHEET,HEADERS.StudentPhotos)).forEach(r=>{const id=String(r[0]||'').trim();if(id)map[id]={fileId:String(r[1]||''),photoUrl:String(r[2]||''),updatedAt:String(r[3]||'')}});return map}
function athleteRows_(){const sports={};rows_(sheet_(SPORTS_SHEET,HEADERS.Sports)).forEach(r=>sports[String(r[0])]=String(r[1]||''));const photos=photoMap_();return rows_(sheet_(ATHLETES_SHEET,HEADERS.Athletes)).filter(r=>r[0]&&r[2]).map(r=>({sportId:String(r[0]),sportName:sports[String(r[0])]||'',color:String(r[1]||''),studentId:String(r[2]),studentName:String(r[3]||''),levelRoom:String(r[4]||''),photoUrl:(photos[String(r[2])]||{}).photoUrl||''}))}
function getPublicAthletes(){return{athletes:athleteRows_()}}
function requireStaffAthlete_(color,staffKey,studentId){if(COLORS.indexOf(color)<0)throw new Error('สีไม่ถูกต้อง');requireSecret_('STAFF_'+color.toUpperCase()+'_KEY',staffKey,' Staff');const athlete=athleteRows_().find(a=>a.color===color&&a.studentId===String(studentId));if(!athlete)throw new Error('จัดการรูปได้เฉพาะนักกีฬาในสีของคุณ');return athlete}
function athletePhotoFolder_(){const properties=PropertiesService.getScriptProperties(),saved=properties.getProperty('ATHLETE_PHOTO_FOLDER_ID');if(saved){try{return DriveApp.getFolderById(saved)}catch(e){}}const folder=DriveApp.createFolder('BHP Sports Athlete Photos');properties.setProperty('ATHLETE_PHOTO_FOLDER_ID',folder.getId());return folder}
function saveStudentPhoto(p){const studentId=String(p.studentId||'').trim(),mime=String(p.mimeType||'');requireStaffAthlete_(String(p.color||''),p.staffKey,studentId);if(['image/jpeg','image/png','image/webp'].indexOf(mime)<0)throw new Error('รองรับเฉพาะ JPG, PNG และ WebP');const bytes=Utilities.base64Decode(String(p.base64||''));if(!bytes.length||bytes.length>1500000)throw new Error('รูปมีขนาดใหญ่เกิน 1.5 MB');const photos=photoMap_(),old=photos[studentId];if(old&&old.fileId){try{DriveApp.getFileById(old.fileId).setTrashed(true)}catch(e){}}const ext=mime==='image/png'?'.png':mime==='image/webp'?'.webp':'.jpg',file=athletePhotoFolder_().createFile(Utilities.newBlob(bytes,mime,studentId+ext));file.setSharing(DriveApp.Access.ANYONE_WITH_LINK,DriveApp.Permission.VIEW);const url='https://drive.google.com/thumbnail?id='+file.getId()+'&sz=w400',sh=sheet_(STUDENT_PHOTOS_SHEET,HEADERS.StudentPhotos),data=rows_(sh),idx=data.findIndex(r=>String(r[0])===studentId),row=[studentId,file.getId(),url,now_()];if(idx>=0)sh.getRange(idx+2,1,1,4).setValues([row]);else sh.appendRow(row);audit_('SAVE_STUDENT_PHOTO','Staff',{studentId:studentId,color:p.color});return{success:true,photoUrl:url}}
function deleteStudentPhoto(p){const id=String(p.studentId||'');requireStaffAthlete_(String(p.color||''),p.staffKey,id);const sh=sheet_(STUDENT_PHOTOS_SHEET,HEADERS.StudentPhotos),data=rows_(sh),idx=data.findIndex(r=>String(r[0])===id);if(idx>=0){try{DriveApp.getFileById(String(data[idx][1])).setTrashed(true)}catch(e){}sh.deleteRow(idx+2)}audit_('DELETE_STUDENT_PHOTO','Staff',{studentId:id,color:p.color});return{success:true}}
...
function setStaffRegistrationOpen(open,adminKey,userType){requireSecret_('ADMIN_KEY',adminKey,'ผู้ดูแลระบบ');const value=open===true||String(open).toLowerCase()==='true',sh=sheet_(SETTINGS_SHEET,HEADERS.Settings),data=rows_(sh),idx=data.findIndex(r=>String(r[0])==='StaffRegistrationOpen'),row=['StaffRegistrationOpen',value,now_()];if(idx>=0)sh.getRange(idx+2,1,1,3).setValues([row]);else sh.appendRow(row);audit_(value?'OPEN_STAFF_REGISTRATION':'CLOSE_STAFF_REGISTRATION',userType||'Admin',{open:value});return{success:true,staffRegistrationOpen:value}}
/** Return only the authenticated team's roster, athletes, and relevant matches. */
function getStaffData(color,staffKey){if(COLORS.indexOf(color)<0)throw new Error('สีไม่ถูกต้อง');requireSecret_('STAFF_'+color.toUpperCase()+'_KEY',staffKey,' Staff');const competition=getCompetitionData(),teachers=getTeachers().teachers[color]||[],photos=photoMap_(),students=studentObjects_().filter(s=>s.color===color).map(s=>Object.assign(s,{photoUrl:(photos[s.id]||{}).photoUrl||''})),athletes=rows_(sheet_(ATHLETES_SHEET,HEADERS.Athletes)).filter(r=>String(r[1])===color).map(r=>({sportId:String(r[0]),studentId:String(r[2]),studentName:String(r[3]),levelRoom:String(r[4]),updatedAt:String(r[5]||''),photoUrl:(photos[String(r[2])]||{}).photoUrl||''})),belongs=team=>String(team||'').split('-').indexOf(color)>=0;return{color:color,staffRegistrationOpen:staffRegistrationOpen_(),students:students,teachers:teachers,sports:competition.sports,athletes:athletes,matches:competition.matches.filter(m=>belongs(m.teamA)||belongs(m.teamB)||belongs(m.winner)||belongs(m.loser))}}
...
function getColorStudents(color,sportId){if(COLORS.indexOf(color)<0)throw new Error('สีไม่ถูกต้อง');const sports=getCompetitionData().sports,sport=sports.find(s=>s.id===sportId);if(!sport)throw new Error('ไม่พบกีฬา');const photos=photoMap_(),students=studentObjects_().filter(s=>s.color===color&&sportAllowsLevel_(sport.level,s.level)&&sportAllowsGender_(sport.gender,s.prefix)).map(s=>Object.assign(s,{photoUrl:(photos[s.id]||{}).photoUrl||''}));const selectedIds=rows_(sheet_(ATHLETES_SHEET,HEADERS.Athletes)).filter(r=>String(r[0])===sportId&&String(r[1])===color).map(r=>String(r[2]));return{students:students,selectedIds:selectedIds,limit:sport.athleteLimit,levels:sport.level,gender:sport.gender}}
function saveAthletes(sportId,color,studentIds,userType,staffKey){if(COLORS.indexOf(color)<0)throw new Error('สีไม่ถูกต้อง');if(userType==='Staff')requireSecret_('STAFF_'+color.toUpperCase()+'_KEY',staffKey,' Staff');if(!staffRegistrationOpen_())throw new Error('แอดมินปิดระบบลงทะเบียนนักกีฬาแล้ว');if(!Array.isArray(studentIds))throw new Error('ข้อมูลนักกีฬาไม่ถูกต้อง');const sport=getCompetitionData().sports.find(s=>s.id===sportId);if(!sport)throw new Error('ไม่พบกีฬา');if(studentIds.length>sport.athleteLimit)throw new Error('เลือกนักกีฬาได้สูงสุด '+sport.athleteLimit+' คน');const allowed=studentObjects_().filter(s=>s.color===color&&sportAllowsLevel_(sport.level,s.level)&&sportAllowsGender_(sport.gender,s.prefix)),map={};allowed.forEach(s=>map[s.id]=s);studentIds.forEach(id=>{if(!map[String(id)])throw new Error('พบนักเรียนที่ไม่ตรงกับสี เพศ หรือระดับชั้นของกีฬา')});const lock=LockService.getScriptLock();lock.waitLock(10000);try{if(!staffRegistrationOpen_())throw new Error('แอดมินปิดระบบลงทะเบียนนักกีฬาแล้ว');const sh=sheet_(ATHLETES_SHEET,HEADERS.Athletes),existing=rows_(sh),kept=existing.filter(r=>!(String(r[0])===sportId&&String(r[1])===color)),stamp=now_(),replacement=studentIds.map(id=>{const s=map[String(id)];return[sportId,color,s.id,(s.prefix||'')+s.firstName+' '+s.lastName,s.level+'/'+s.room,stamp]}),output=kept.concat(replacement),oldCount=Math.max(0,sh.getLastRow()-1);if(oldCount)sh.getRange(2,1,oldCount,HEADERS.Athletes.length).clearContent();if(output.length)sh.getRange(2,1,output.length,HEADERS.Athletes.length).setValues(output);audit_('SAVE_ATHLETES',userType||'Color Manager',{sportId:sportId,color:color,levels:sport.level,gender:sport.gender,count:studentIds.length});return{success:true,saved:studentIds.length,updatedAt:stamp}}finally{lock.releaseLock()}}
