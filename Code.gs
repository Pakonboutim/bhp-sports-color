...
function getAthletePhotoData(adminKey){requireSecret_('ADMIN_KEY',adminKey,'ผู้ดูแลระบบ');const grouped={};athleteRows_().forEach(a=>{if(!grouped[a.studentId])grouped[a.studentId]={studentId:a.studentId,studentName:a.studentName,levelRoom:a.levelRoom,color:a.color,photoUrl:a.photoUrl,sports:[]};if(grouped[a.studentId].sports.indexOf(a.sportName)<0)grouped[a.studentId].sports.push(a.sportName)});return{athletes:Object.values(grouped)}}
...
function saveStudentPhoto(p){requireSecret_('ADMIN_KEY',p.adminKey,'ผู้ดูแลระบบ');const studentId=String(p.studentId||'').trim(),mime=String(p.mimeType||'');if(!studentObjects_().some(s=>s.id===studentId))throw new Error('ไม่พบนักเรียน');if(['image/jpeg','image/png','image/webp'].indexOf(mime)<0)throw new Error('รองรับเฉพาะ JPG, PNG และ WebP');const bytes=Utilities.base64Decode(String(p.base64||''));if(!bytes.length||bytes.length>1500000)throw new Error('รูปมีขนาดใหญ่เกิน 1.5 MB');const photos=photoMap_(),old=photos[studentId];if(old&&old.fileId){try{DriveApp.getFileById(old.fileId).setTrashed(true)}catch(e){}}const ext=mime==='image/png'?'.png':mime==='image/webp'?'.webp':'.jpg',file=athletePhotoFolder_().createFile(Utilities.newBlob(bytes,mime,studentId+ext));file.setSharing(DriveApp.Access.ANYONE_WITH_LINK,DriveApp.Permission.VIEW);const url='https://drive.google.com/thumbnail?id='+file.getId()+'&sz=w400',sh=sheet_(STUDENT_PHOTOS_SHEET,HEADERS.StudentPhotos),data=rows_(sh),idx=data.findIndex(r=>String(r[0])===studentId),row=[studentId,file.getId(),url,now_()];if(idx>=0)sh.getRange(idx+2,1,1,4).setValues([row]);else sh.appendRow(row);audit_('SAVE_STUDENT_PHOTO','Admin',{studentId:studentId});return{success:true,photoUrl:url}}
function deleteStudentPhoto(p){requireSecret_('ADMIN_KEY',p.adminKey,'ผู้ดูแลระบบ');const id=String(p.studentId||''),sh=sheet_(STUDENT_PHOTOS_SHEET,HEADERS.StudentPhotos),data=rows_(sh),idx=data.findIndex(r=>String(r[0])===id);if(idx>=0){try{DriveApp.getFileById(String(data[idx][1])).setTrashed(true)}catch(e){}sh.deleteRow(idx+2)}audit_('DELETE_STUDENT_PHOTO','Admin',{studentId:id});return{success:true}}
