...
function addButtons(){
  const root=document.getElementById('print-classroom-checks');if(!root)return;
  const tools=root.parentElement?.querySelector('.print-check-tools');if(!tools||tools.querySelector('[data-admin-print-group]'))return;
  GROUPS.forEach(g=>{const b=document.createElement('button');b.type='button';b.className='btn';b.dataset.adminPrintGroup=g.id;b.textContent=g.label;b.onclick=()=>{root.querySelectorAll('input').forEach(x=>x.checked=g.levels.includes(String(x.value).split('|')[0]))};tools.appendChild(b)});
}
